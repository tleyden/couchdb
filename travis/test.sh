...
case "$1" in
    ...
    "coverage")
        ...
        TEST_PKGS=$(find -name "*_test.go" | grep -v /vendor/ | grep -v /test/ | xargs dirname | sort -u | sed -e "s#^\.#github.com/tleyden/couchdb#" )

        for d in $TEST_PKGS; do
            go test -i $d
            DEPS=$((go list -f $'{{range $f := .TestImports}}{{$f}}\n{{end}}{{range $f := .Imports}}{{$f}}\n{{end}}' $d && echo $d) | sort -u | grep -v /vendor/ | grep ^github.com/tleyden/couchdb | tr '\n' ' ')
